...
from shopozor.emails import send_activate_account_email, send_hacker_abuse_email_notification, send_password_reset, get_email_base_context
...
def test_send_password_reset_email(customer_user):
    send_password_reset(customer_user)
    assert len(mail.outbox) == 1
...
def test_password_reset_email_correct_url(customer_user):
    send_password_reset(customer_user)
    mail_sent = mail.outbox[0].body
