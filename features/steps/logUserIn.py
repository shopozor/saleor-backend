...
def step_impl(context):
    ...
    variables = {'email': context.consumer['email'],
                 'password': context.consumer['password'], 'isStaff': True}
...
def step_impl(context, user_type, pretended_type, validity):
    use_fixture(graphql_query, context, 'login.graphql')
    if validity:
        variables = valid_mail_and_password(
            user_type, is_staff(pretended_type), context)
    else:
        variables = invalid_mail_and_password(
            is_staff(pretended_type), context)
    response = context.test.client.post_graphql(context.query, variables)
    context.response = get_graphql_content(response)
...
def step_impl(context, user_type, pretended_type):
    use_fixture(graphql_query, context, 'login.graphql')
    variables = valid_mail_invalid_password(
        user_type, is_staff(pretended_type), context)
    response = context.test.client.post_graphql(context.query, variables)
    context.response = get_graphql_content(response)
